...
// startOutboxDispatcher spins up the polling loop in a background goroutine
// and registers cancellation + producer close as OnShutdown hooks.
func startOutboxDispatcher(a *app.App, deps app.Dependencies) {
	brokers := deps.Config.Kafka.Brokers
	if len(brokers) == 0 {
		slog.Warn("kafka brokers not configured — outbox dispatcher disabled")
		return
	}
