...
// startOutboxDispatcher spins up the store outbox polling loop in a background
...
func startOutboxDispatcher(a *app.App, deps app.Dependencies) {
	...
	if len(brokers) == 0 {
		slog.Warn("kafka brokers not configured — store outbox dispatcher disabled")
		return
	}
	...
	a.OnShutdown(func() {
		...
		if err := producer.Close(); err != nil {
			slog.Error("store: kafka producer close error", "err", err)
		}
	})
}
