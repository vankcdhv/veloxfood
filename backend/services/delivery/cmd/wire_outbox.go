...
// startOutboxDispatcher spins up the delivery outbox polling loop and registers
// cancellation + producer close as OnShutdown hooks.
func startOutboxDispatcher(a *app.App, deps app.Dependencies) {
	brokers := deps.Config.Kafka.Brokers
	if len(brokers) == 0 {
		slog.Warn("kafka brokers not configured — delivery outbox dispatcher disabled")
		return
	}
	...
	a.OnShutdown(func() {
		...
		if err := producer.Close(); err != nil {
			slog.Error("delivery: kafka producer close error", "err", err)
		}
	})
}
