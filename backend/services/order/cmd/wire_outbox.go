...
// startOutboxDispatcher spins up the order outbox polling loop and registers
...
func startOutboxDispatcher(a *app.App, deps app.Dependencies) {
	...
	if len(brokers) == 0 {
		slog.Warn("kafka brokers not configured — order outbox dispatcher disabled")
		return
	}
	...
	a.OnShutdown(func() {
		...
		if err := producer.Close(); err != nil {
			slog.Error("order: kafka producer close error", "err", err)
		}
	})
}
